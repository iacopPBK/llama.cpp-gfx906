...
#if defined(GGML_USE_HIP)
    // Use optimized kernels on AMD when memory layout permits
    if (gfx906_add_id_can_use_optimized(ne00, nb01, nb11, nb10)) {
        gfx906_add_id_dispatch(src0_d, src1_d, src2_d, dst_d,
            ne0, ne1, ne00, ne01, ne02,
            nb01, nb02, nb11, nb21, nb10,
            ctx.stream());
    } else
#endif
    {
