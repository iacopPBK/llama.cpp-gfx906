...
// GFX906-optimized DS_SWIZZLE reductions using compiler intrinsics for optimal performance
...
__device__ __forceinline__ T wave_reduce_sum(T value) {
    if constexpr (sizeof(T) == 4) {
        ...
        if (warp_size == 64) {
            value += __shfl_xor(value, 32, 64);  // Swap between wave halves
        }
    }
    return value;
}
...
// Override standard functions with GFX906 DS_SWIZZLE optimized versions
#define warp_reduce_sum ::gfx906::wave_reduce_sum
#define warp_reduce_max ::gfx906::wave_reduce_max
