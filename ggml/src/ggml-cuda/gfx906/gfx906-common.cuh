...
// ================================================================================================
// Optimized 32-thread (half-warp) reduction using fully fused DPP instructions
// This is used by MMVQ kernels for maximum performance on small matrices
// ================================================================================================

// Sum reduction for 32 threads - uses fused DPP for all steps
static __device__ __forceinline__ float gfx906_warp_reduce_sum_32_f32(float x) {
    float result;
    asm volatile(
        // Step 1: xor1 - adjacent lane exchange (offset 1)
        "s_nop 4\n"
        "v_add_f32_dpp %0, %1, %1 quad_perm:[1,0,3,2] row_mask:0xf bank_mask:0xf\n"
        // Step 2: xor2 - pair exchange (offset 2)  
        "s_nop 1\n"
        "v_add_f32_dpp %0, %0, %0 quad_perm:[2,3,0,1] row_mask:0xf bank_mask:0xf\n"
        // Step 3: xor4 - nibble shift (offset 4) - FUSED instead of separate shuffle+add
        "s_nop 1\n"
        "v_add_f32_dpp %0, %0, %0 row_shl:4 row_mask:0xf bank_mask:0x5\n"
        "v_add_f32_dpp %0, %0, %0 row_shr:4 row_mask:0xf bank_mask:0xa\n"
        // Step 4: xor8 - byte rotate (offset 8)
        "s_nop 1\n"
        "v_add_f32_dpp %0, %0, %0 row_ror:8 row_mask:0xf bank_mask:0xf\n"
        // Step 5: xor16 - half-warp swap (offset 16) - use ds_swizzle for this
        "ds_swizzle_b32 %0, %0 offset:swizzle(SWAP,16)\n"
        "s_waitcnt lgkmcnt(0)\n"
        "v_add_f32 %0, %0, %0\n"
        : "=v"(result) : "v"(x) : "memory"
    );
...
// Max reduction for 32 threads - uses fused DPP for all steps
static __device__ __forceinline__ float gfx906_warp_reduce_max_32_f32(float x) {
    float result;
    asm volatile(
        // Step 1: xor1 - adjacent lane exchange (offset 1)
        "s_nop 4\n"
        ...
        // Step 3: xor4 - nibble shift (offset 4) - FUSED instead of separate shuffle+max
        "s_nop 1\n"
        "v_max_f32_dpp %0, %0, %0 row_shl:4 row_mask:0xf bank_mask:0x5\n"
        "v_max_f32_dpp %0, %0, %0 row_shr:4 row_mask:0xf bank_mask:0xa\n"
        // Step 4: xor8 - byte rotate (offset 8)
        "s_nop 1\n"
        "v_max_f32_dpp %0, %0, %0 row_ror:8 row_mask:0xf bank_mask:0xf\n"
        // Step 5: xor16 - half-warp swap (offset 16)
        "ds_swizzle_b32 %0, %0 offset:swizzle(SWAP,16)\n"
        "s_waitcnt lgkmcnt(0)\n"
        "v_max_f32 %0, %0, %0\n"
        : "=v"(result) : "v"(x) : "memory"
