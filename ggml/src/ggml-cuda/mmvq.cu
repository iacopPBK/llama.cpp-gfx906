...
#include "gfx906/mmvq-dispatch.cuh"
...
static void mul_mat_vec_q_switch_type(
    ...
    switch (type_x) {
        case GGML_TYPE_Q4_0:
            // Try GFX906 warp-cooperative kernel first (returns false if not applicable or on non-HIP)
            if (gfx906_mmvq_try_q4_0(vx, vy, ids, dst, ncols_x, ncols_dst, nchannels_y, stride_row_x, stride_col_dst,
        ...
        case GGML_TYPE_Q4_1:
            // Try GFX906 warp-cooperative kernel first (returns false if not applicable or on non-HIP)
            if (gfx906_mmvq_try_q4_1(vx, vy, ids, dst, ncols_x, ncols_dst, nchannels_y, stride_row_x, stride_col_dst,
                    nchannels_dst / nchannels_x, stride_channel_x, stride_channel_y, stride_channel_dst,
                    nsamples_dst / nsamples_x, stride_sample_x, stride_sample_y, stride_sample_dst,
                    nrows_x, nchannels_dst, nsamples_dst, fusion, stream)) {
                break;
        ...
        case GGML_TYPE_Q8_0:
            // Try GFX906 warp-cooperative kernel first (returns false if not applicable or on non-HIP)
            if (gfx906_mmvq_try_q8_0(vx, vy, ids, dst, ncols_x, ncols_dst, nchannels_y, stride_row_x, stride_col_dst,
                    nchannels_dst / nchannels_x, stride_channel_x, stride_channel_y, stride_channel_dst,
                    nsamples_dst / nsamples_x, stride_sample_x, stride_sample_y, stride_sample_dst,
                    nrows_x, nchannels_dst, nsamples_dst, fusion, stream)) {
                break;
